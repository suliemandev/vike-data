// The default b2c-subscription instance: an in-memory universal-orm repository over
// the `subscriptions` table + the subscription core, cached on globalThis so
// duplicate module evaluation (pointer imports, dev HMR) can't fork the store.
//
...
const KEY = Symbol.for('vike-stripe.b2c-subscription.instance')
...
if (!globalThis[KEY]) {
  ...
  const { tables } = mergeSchemas(subscriptionSchemas({ billingSubject: subject }))
  const db = createRepository({ tables }, createMemoryAdapter())
  globalThis[KEY] = createSubscriptions({ db, subject })
}
