...
export function generateModule(locales, registry) {
  ...
  return [
    '// GENERATED by vike-i18n/plugin from `locales` + the `localePacks` registry.',
    ...imports,
...
export function vikeI18n() {
  ...
  return {
    ...
    buildStart() {
      ...
      this.info?.(
        `[vike-i18n] locales=[${config.locales.join(', ')}] -> ${generateModule(config.locales, config.registry)
          .split('\n')
    ...
    load(id) {
      ...
      return generateModule(config.locales, config.registry)
    },
  }
}
