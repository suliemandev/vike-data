// The Vite plugin that makes zero-config `lang: [...]` auto-include work (#79).
//
...
//
//   lang: ['en', 'fr', 'ar']   // auto-includes vike-auth/fr + /ar, billing/ar, ...
//
...
// resolution. This plugin reads the app's resolved `lang` + the merged registry
...
// is in `lang`. Vite still tree-shakes per locale (an unused locale is never
...
// Read the app's `lang` + the merged `localePacks` off the resolved Vike config.
...
export function generateModule(lang, registry) {
  const wanted = new Set(lang.filter((l) => l !== 'en'))
  const imports = []
  const names = []
  let i = 0
  ...
  return [
    '// GENERATED by vike-i18n/plugin from `lang` + the `localePacks` registry.',
    ...imports,
...
export function vikeI18n() {
  ...
  return {
    ...
    // Recompute on every (re)load so adding a language or installing an extension
    // is picked up; the work is cheap (a couple of string lookups + concat).
    buildStart() {
      config = readI18nConfig()
      this.info?.(
        `[vike-i18n] lang=[${config.lang.join(', ')}] -> ${generateModule(config.lang, config.registry)
          .split('\n')
          .filter((l) => l.startsWith('import')).length} pack(s) bundled`,
      )
    },
    resolveId(id) {
    ...
    load(id) {
      ...
      if (!config.lang) config = readI18nConfig()
      return generateModule(config.lang, config.registry)
    },
  }
}
