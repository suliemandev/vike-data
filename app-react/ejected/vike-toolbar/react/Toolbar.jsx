// EJECTED + app-owned (see ../EJECT.md). This is a copy of vike-toolbar's source that
// the app took ownership of. The app-owned edit below (a branded panel header) is the
// kind of long-tail change you make by ejecting instead of by adding a config knob.
//
      ...
      {root &&
        createPortal(
          <div role="dialog" aria-label="Settings" style={{ ...panelStyle, display: open ? 'flex' : 'none' }}>
            {/* App-owned header: only possible because we ejected and own this source. */}
            <div style={{ fontWeight: 600, letterSpacing: '0.02em', color: 'var(--color-text)', paddingBottom: 4, borderBottom: '1px solid var(--color-border)' }}>
              Acme settings
            </div>
