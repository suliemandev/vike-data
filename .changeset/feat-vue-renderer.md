...
Vue renderers as `vike-blocks/vue` + `vike-view/vue` subpaths — the Vue twin of the React binding, in the same packages (matching vike-auth's `./react` + `./vue` convention, no separate `vike-vue-*` package).

- **vike-blocks/vue**: the block-renderer registry (`registerBlockRenderer` over the shared 'blocks'/'vue' slot), the `<Blocks>`/`<Page>` dispatch, and the built-in primitive components (Text / Heading / Badge / Divider / Link / Markdown / Stat).
...
The Vue components are written as functional components (a function of props with an explicit `.props`), the closest Vue analog to the React function components, so no `.vue` compile step. Verified end to end via `@vue/server-renderer`: a `defineView` of blocks + `crudBlocks` server-renders to HTML with the schema driving the list table, record fields, and form controls (enum → `<select>`, boolean → checkbox).
